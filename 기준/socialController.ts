...
export class SocialController implements Controller {
  ...
  public async facebookCallback(
    ...
    const profile =
      await this.socialService.getSocialDataFromFacebook(
        code,
      );
  ...
  public async githubCallback(
    ...
    const profile =
      await this.socialService.getSocialDataFromGithub(
        code,
      );
    request.socialProfile = profile;
  }
  public async googleCallback(
    ...
    const profile =
      await this.socialService.getSocialDataFromGoogle(
        code,
      );
    request.socialProfile = profile;
  }
  public async socialCallback({
    ...
    if (!socialProfile) {
      throw new NotFoundError(
        "Social profile data is missing",
      );
    }
    ...
    if (socialAccount || profile.email) {
      ...
      if (user) {
        const tokens = await this.jwt.generateUserToken(
          user.id,
        );
        ...
        if (next.includes("user-integrate") && state) {
          const isIntegrated =
            await this.externalInterationService.checkIntegrated(
              user.id,
            );
          if (isIntegrated) {
            const code =
              await this.externalInterationService.createIntegrationCode(
                user.id,
              );
            reply.redirect(
              `${ENV.codenaryCallback}?code=${code}&state=${state.integrateState}`,
            );
            return;
          }
        }
        ...
        const redirectUri = this.utils
          .removeKoreanChars(
            decodeURI(redirectHost.concat(next)),
          )
          .trim();
    ...
    const registerToken = await this.jwt.generateToken(
      registerTokenInfo,
      {
        expiresIn: "1h",
      },
  ...
  public async socialRegister(
    ...
    const registerToken = this.cookie.getCookie(
      request,
      "register_token",
    );
    if (!registerToken) {
    ...
    const { display_name, username, short_bio } =
      request.body;
    let decoded: SocialRegisterToken | null = null;
    try {
      decoded =
        await this.jwt.decodeToken<SocialRegisterToken>(
          registerToken,
        );
    } catch (e) {
      throw new UnauthorizedError();
    ...
    if (decoded?.profile.thumbnail) {
      const imageUrl = await this.syncProfileImageWithB2(
        decoded.profile.thumbnail,
        user,
      );
  ...
  private async syncProfileImageWithB2(
    url: string,
    user: User,
  ): Promise<string> {
    ...
    const buffer = await new Promise<Buffer>(
      (resolve, reject) => {
        ...
        result.stream.on("data", (data: Buffer) =>
          buffers.push(data),
        );
    ...
    const uploadResult = await this.b2Manager.upload(
      buffer,
      uploadPath,
    );
  ...
  public async socialRedirect({
    ...
    const loginUrl = this.generateSocialLoginLink(
      provider,
      {
  ...
  private generateSocialLoginLink(
    ...
    const generator =
      this.socialLoginLinkGenerator[provider];
    return generator({
      next: encodeURI(next),
      isIntegrate,
      integrateState,
